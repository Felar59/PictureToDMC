const k=new Float64Array(256);for(let e=0;e<256;e++){const r=e/255;k[e]=r<=.04045?r/12.92:((r+.055)/1.055)**2.4}const v=.95047,b=1.08883,T=.008856,p=7.787;function L(e){return e>T?Math.cbrt(e):p*e+16/116}function V(e,r,n){const a=k[e<0?0:e>255?255:e|0],t=k[r<0?0:r>255?255:r|0],l=k[n<0?0:n>255?255:n|0],u=L((a*.4124+t*.3576+l*.1805)/v),o=L(a*.2126+t*.7152+l*.0722),s=L((a*.0193+t*.1192+l*.9505)/b);return[116*o-16,500*(u-o),200*(o-s)]}function m(e,r){const n=e[0]-r[0],a=e[1]-r[1],t=e[2]-r[2];return n*n+a*a+t*t}function R(e){const r=parseInt(e.replace("#",""),16);return[r>>16&255,r>>8&255,r&255]}function q(e,r){if(r<=0)return;const n=1+r/100*.6;for(let a=0;a<e.length;a+=3)e[a+1]*=n,e[a+2]*=n}const O=64,U=900,x=484,I=.06;function W(e,r,n,a){const t=n*a;if(t===0)return!1;const l=D=>[e[D*3],e[D*3+1],e[D*3+2]],u=[];for(let D=0;D<n;D++)u.push(D,(a-1)*n+D);for(let D=1;D<a-1;D++)u.push(D*n,D*n+n-1);const o=u.filter(D=>r[D]!==0);if(o.length===0)return!1;const s=H(e,o),A=new Int32Array(t),B=new Uint8Array(t);let i=0,c=0;for(const D of o)!B[D]&&m(l(D),s)<=x&&(B[D]=1,A[c++]=D);if(c===0)return!1;const g=[];for(;i<c;){const D=A[i++];g.push(D);const h=l(D),y=D%n,f=(D-y)/n;y>0&&d(D-1,h),y<n-1&&d(D+1,h),f>0&&d(D-n,h),f<a-1&&d(D+n,h)}function d(D,h){if(B[D]||r[D]===0)return;const y=l(D);m(y,h)>O||m(y,s)>U||(B[D]=1,A[c++]=D)}const C=N(r);if(C-g.length<C*I)return!1;for(const D of g)r[D]=0;return z(e,r,n,a,s),!0}function z(e,r,n,a,t){const l=o=>[e[o*3],e[o*3+1],e[o*3+2]],u=[];for(let o=0;o<a;o++)for(let s=0;s<n;s++){const A=o*n+s;if(r[A]===0)continue;let B=!1,i=[0,0,0],c=0;const g=F=>{if(r[F]===0)B=!0;else{const D=l(F);i=[i[0]+D[0],i[1]+D[1],i[2]+D[2]],c++}};if(s>0&&g(A-1),s<n-1&&g(A+1),o>0&&g(A-n),o<a-1&&g(A+n),!B||c===0)continue;const d=[i[0]/c,i[1]/c,i[2]/c],C=l(A);m(C,t)<m(C,d)&&u.push(A)}for(const o of u)r[o]=0}function N(e){let r=0;for(let n=0;n<e.length;n++)e[n]!==0&&r++;return r}function H(e,r){const n=a=>{const t=r.map(u=>e[u*3+a]).sort((u,o)=>u-o),l=t.length>>1;return t.length%2?t[l]:(t[l-1]+t[l])/2};return[n(0),n(1),n(2)]}function Y(e,r){const n=new Float64Array(r*3);for(let a=0;a<r;a++){const t=V(e[a*4],e[a*4+1],e[a*4+2]);n[a*3]=t[0],n[a*3+1]=t[1],n[a*3+2]=t[2]}return n}const E=320,G=[.485,.456,.406],M=[.229,.224,.225];let S=null;function K(){return S??=(async()=>{const[e,r,n]=await Promise.all([import("./ort.wasm.bundle.min-gveYohPt.js"),import("./ort-wasm-simd-threaded-BkColDmu.js").then(l=>l.default),import("./ort-wasm-simd-threaded-C07VKIhp.js").then(l=>l.default)]),a="/models/u2netp.onnx";return e.env.wasm.numThreads=1,e.env.wasm.wasmPaths={wasm:r,mjs:n},{session:await e.InferenceSession.create(a,{executionProviders:["wasm"],graphOptimizationLevel:"all"}),Tensor:e.Tensor}})(),S}function _(){return typeof WebAssembly=="object"&&typeof OffscreenCanvas=="function"}async function J(e){const{session:r,Tensor:n}=await K(),a=await createImageBitmap(e),l=new OffscreenCanvas(E,E).getContext("2d",{willReadFrequently:!0});if(!l)throw a.close(),new Error("canvas 2d context unavailable");l.imageSmoothingEnabled=!0,l.imageSmoothingQuality="high",l.drawImage(a,0,0,a.width,a.height,0,0,E,E),a.close();const u=l.getImageData(0,0,E,E).data,o=new Float32Array(3*E*E),s=E*E;for(let C=0;C<s;C++)o[C]=(u[C*4]/255-G[0])/M[0],o[s+C]=(u[C*4+1]/255-G[1])/M[1],o[s*2+C]=(u[C*4+2]/255-G[2])/M[2];const B=(await r.run({[r.inputNames[0]]:new n("float32",o,[1,3,E,E])}))[r.outputNames[0]].data;let i=1/0,c=-1/0;for(let C=0;C<s;C++)B[C]<i&&(i=B[C]),B[C]>c&&(c=B[C]);const g=Math.max(1e-6,c-i),d=new Float32Array(s);for(let C=0;C<s;C++)d[C]=(B[C]-i)/g;return d}function $(e,r,n,a,t={}){const l=t.threshold??.5;for(let u=0;u<n;u++){const o=Math.floor(u/n*E),s=Math.max(o+1,Math.floor((u+1)/n*E));for(let A=0;A<r;A++){const B=u*r+A;if(e[B]===0)continue;const i=Math.floor(A/r*E),c=Math.max(i+1,Math.floor((A+1)/r*E));let g=0,d=0;for(let C=o;C<s&&C<E;C++){const F=t.flipV?E-1-C:C;for(let D=i;D<c&&D<E;D++){const h=t.flipH?E-1-D:D;g+=a[F*E+h],d++}}d>0&&g/d<l&&(e[B]=0)}}}const Q=`Ecru|Ecru/off-white|FFF7E7
Blanc|White|FCFCFF
White|White|FCFCFF
...
S3820|Satin - Straw|DBA53E`,P=Q.split(`
`).map(e=>{const[r,n,a]=e.split("|"),t=R(a);return{num:r,name:n,hex:`#${a}`,rgb:t,lab:V(t[0],t[1],t[2])}}),X=new Map(P.map(e=>[e.num.toLowerCase(),e]));function Z(e){return X.get(e.trim().toLowerCase())}function j(e,r=P){const n=e.length;if(n===0)return[];const a=r.length,t=n*a,l=new Float64Array(t);for(let B=0;B<n;B++){const i=e[B];for(let c=0;c<a;c++)l[B*a+c]=m(i,r[c].lab)}const u=new Int32Array(t);for(let B=0;B<t;B++)u[B]=B;u.sort((B,i)=>l[B]-l[i]);const o=new Array(n),s=new Uint8Array(a);let A=0;for(let B=0;B<t&&A<n;B++){const i=u[B],c=i/a|0,g=i-c*a;o[c]||s[g]||(o[c]=r[g],s[g]=1,A++)}for(let B=0;B<n;B++)if(!o[B]){let i=0,c=1/0;for(let g=0;g<r.length;g++){const d=m(e[B],r[g].lab);d<c&&(c=d,i=g)}o[B]=r[i]}return o}function ee(e){let r=e>>>0;return()=>{r=r+1831565813>>>0;let n=r;return n=Math.imul(n^n>>>15,n|1),n^=n+Math.imul(n^n>>>7,n|61),((n^n>>>14)>>>0)/4294967296}}const ne=60,re=42;function te(e,r){const n=e.length/3;if(n===0||r<=0)return{centroids:new Float64Array(0),labels:new Int32Array(0)};const a=ae(e,n);r=Math.min(r,a);const t=ee(re),l=ie(e,n,r,t),u=new Int32Array(n),o=new Float64Array(r*3),s=new Int32Array(r);for(let A=0;A<ne;A++){let B=!1;for(let i=0;i<n;i++){const c=e[i*3],g=e[i*3+1],d=e[i*3+2];let C=0,F=1/0;for(let D=0;D<r;D++){const h=c-l[D*3],y=g-l[D*3+1],f=d-l[D*3+2],w=h*h+y*y+f*f;w<F&&(F=w,C=D)}u[i]!==C&&(u[i]=C,B=!0)}o.fill(0),s.fill(0);for(let i=0;i<n;i++){const c=u[i];o[c*3]+=e[i*3],o[c*3+1]+=e[i*3+1],o[c*3+2]+=e[i*3+2],s[c]++}for(let i=0;i<r;i++)s[i]!==0&&(l[i*3]=o[i*3]/s[i],l[i*3+1]=o[i*3+1]/s[i],l[i*3+2]=o[i*3+2]/s[i]);if(!B)break}return{centroids:l,labels:u}}function ae(e,r){const n=new Set;for(let a=0;a<r;a++)if(n.add(`${e[a*3]|0},${e[a*3+1]|0},${e[a*3+2]|0}`),n.size>64)return n.size;return n.size}function ie(e,r,n,a){const t=new Float64Array(n*3),l=Math.floor(a()*r);t[0]=e[l*3],t[1]=e[l*3+1],t[2]=e[l*3+2];const u=new Float64Array(r).fill(1/0);for(let o=1;o<n;o++){let s=0;for(let i=0;i<r;i++){const c=e[i*3]-t[(o-1)*3],g=e[i*3+1]-t[(o-1)*3+1],d=e[i*3+2]-t[(o-1)*3+2],C=c*c+g*g+d*d;C<u[i]&&(u[i]=C),s+=u[i]}let A=a()*s,B=r-1;for(let i=0;i<r;i++)if(A-=u[i],A<=0){B=i;break}t[o*3]=e[B*3],t[o*3+1]=e[B*3+1],t[o*3+2]=e[B*3+2]}return t}const oe=150;async function le(e,r){const{width:n,height:a,data:t}=await Be(e,r),l=n*a,u=new Int16Array(l).fill(-1),o=Y(t,l),s=new Uint8Array(l);for(let h=0;h<l;h++)s[h]=t[h*4+3]>=oe?255:0;if(r.removeBackground){let h=!1;if(_())try{const y=await J(e);$(s,n,a,y,{flipH:r.flipH,flipV:r.flipV}),h=!0}catch(y){console.warn("cutout unavailable, falling back to the flood fill",y),h=!1}h||W(o,s,n,a)}q(o,r.vividness??0);const A=new Int32Array(l);let B=0;for(let h=0;h<l;h++)s[h]!==0&&(A[B++]=h);if(B===0)return{width:n,height:a,cells:u,threads:[],counts:[],stitched:0};const i=new Float64Array(B*3);for(let h=0;h<B;h++){const y=A[h]*3;i[h*3]=o[y],i[h*3+1]=o[y+1],i[h*3+2]=o[y+2]}const{centroids:c,labels:g}=te(i,r.colorCount),d=c.length/3,C=Array.from({length:d},(h,y)=>[c[y*3],c[y*3+1],c[y*3+2]]),F=j(C,r.palette),D=new Array(d).fill(0);for(let h=0;h<B;h++)u[A[h]]=g[h],D[g[h]]++;return ue({width:n,height:a,cells:u,threads:F,counts:D,stitched:B})}async function Be(e,r){const n=await createImageBitmap(e),a=Math.max(1,Math.round(r.stitchWidth)),t=Math.max(1,Math.round(a*n.height/n.width)),u=new OffscreenCanvas(a,t).getContext("2d",{willReadFrequently:!0});if(!u)throw n.close(),new Error("canvas 2d context unavailable");return u.imageSmoothingEnabled=!0,u.imageSmoothingQuality="high",(r.flipH||r.flipV)&&(u.translate(r.flipH?a:0,r.flipV?t:0),u.scale(r.flipH?-1:1,r.flipV?-1:1)),u.drawImage(n,0,0,n.width,n.height,0,0,a,t),n.close(),{width:a,height:t,data:u.getImageData(0,0,a,t).data}}function De(e){return{width:e.width,height:e.height,cells:e.cells,threadNums:e.threads.map(r=>r.num),counts:e.counts,stitched:e.stitched}}function ue(e){const r=e.threads.map((t,l)=>({i:l,key:ce(t.rgb)})).sort((t,l)=>t.key[0]-l.key[0]||t.key[1]-l.key[1]||t.key[2]-l.key[2]).map(t=>t.i),n=new Int16Array(e.threads.length);r.forEach((t,l)=>{n[t]=l});const a=new Int16Array(e.cells.length);for(let t=0;t<e.cells.length;t++)a[t]=e.cells[t]<0?-1:n[e.cells[t]];return{...e,cells:a,threads:r.map(t=>e.threads[t]),counts:r.map(t=>e.counts[t])}}function ce(e){const[r,n,a]=e.map(s=>s/255),t=Math.max(r,n,a),l=Math.min(r,n,a),u=t-l;let o=0;return u!==0&&(t===r?o=(n-a)/u%6:t===n?o=(a-r)/u+2:o=(r-n)/u+4,o/=6,o<0&&(o+=1)),[o,t===0?0:u/t,t]}self.onmessage=async e=>{const{id:r,photo:n,stitchWidth:a,colorCount:t,paletteNums:l,vividness:u,removeBackground:o,flipH:s,flipV:A}=e.data;try{const B=l?l.map(Z).filter(g=>!!g):void 0,i=await le(n,{stitchWidth:a,colorCount:t,palette:B,vividness:u,removeBackground:o,flipH:s,flipV:A}),c=De(i);self.postMessage({id:r,ok:!0,pattern:c},[c.cells.buffer])}catch(B){self.postMessage({id:r,ok:!1,error:B instanceof Error?B.message:String(B)})}};
