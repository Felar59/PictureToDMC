(function(){"use strict";const F=new Float64Array(256);for(let e=0;e<256;e++){const r=e/255;F[e]=r<=.04045?r/12.92:((r+.055)/1.055)**2.4}const S=.95047,V=1.08883,f=.008856,P=7.787;function s(e){return e>f?Math.cbrt(e):P*e+16/116}function k(e,r,t){const i=F[e<0?0:e>255?255:e|0],n=F[r<0?0:r>255?255:r|0],o=F[t<0?0:t>255?255:t|0],B=s((i*.4124+n*.3576+o*.1805)/S),l=s(i*.2126+n*.7152+o*.0722),u=s((i*.0193+n*.1192+o*.9505)/V);return[116*l-16,500*(B-l),200*(l-u)]}function L(e,r){const t=e[0]-r[0],i=e[1]-r[1],n=e[2]-r[2];return t*t+i*i+n*n}function b(e){const r=parseInt(e.replace("#",""),16);return[r>>16&255,r>>8&255,r&255]}const G=`Ecru|Ecru/off-white|FFF7E7
Blanc|White|FCFCFF
White|White|FCFCFF
...
`).map(e=>{const[r,t,i]=e.split("|"),n=b(i);return{num:r,name:t,hex:`#${i}`,rgb:n,lab:k(n[0],n[1],n[2])}}),v=new Map(G.map(e=>[e.num.toLowerCase(),e]));function T(e){return v.get(e.trim().toLowerCase())}function R(e,r=G){const t=e.length;if(t===0)return[];const i=r.length,n=t*i,o=new Float64Array(n);for(let D=0;D<t;D++){const a=e[D];for(let C=0;C<i;C++)o[D*i+C]=L(a,r[C].lab)}const B=new Int32Array(n);for(let D=0;D<n;D++)B[D]=D;B.sort((D,a)=>o[D]-o[a]);const l=new Array(t),u=new Uint8Array(i);let A=0;for(let D=0;D<n&&A<t;D++){const a=B[D],C=a/i|0,E=a-C*i;l[C]||u[E]||(l[C]=r[E],u[E]=1,A++)}for(let D=0;D<t;D++)if(!l[D]){let a=0,C=1/0;for(let E=0;E<r.length;E++){const g=L(e[D],r[E].lab);g<C&&(C=g,a=E)}l[D]=r[a]}return l}function p(e){let r=e>>>0;return()=>{r=r+1831565813>>>0;let t=r;return t=Math.imul(t^t>>>15,t|1),t^=t+Math.imul(t^t>>>7,t|61),((t^t>>>14)>>>0)/4294967296}}const q=60,O=42;function U(e,r){const t=e.length/3;if(t===0||r<=0)return{centroids:new Float64Array(0),labels:new Int32Array(0)};const i=W(e,t);r=Math.min(r,i);const n=p(O),o=I(e,t,r,n),B=new Int32Array(t),l=new Float64Array(r*3),u=new Int32Array(r);for(let A=0;A<q;A++){let D=!1;for(let a=0;a<t;a++){const C=e[a*3],E=e[a*3+1],g=e[a*3+2];let d=0,h=1/0;for(let c=0;c<r;c++){const y=C-o[c*3],m=E-o[c*3+1],M=g-o[c*3+2],w=y*y+m*m+M*M;w<h&&(h=w,d=c)}B[a]!==d&&(B[a]=d,D=!0)}l.fill(0),u.fill(0);for(let a=0;a<t;a++){const C=B[a];l[C*3]+=e[a*3],l[C*3+1]+=e[a*3+1],l[C*3+2]+=e[a*3+2],u[C]++}for(let a=0;a<r;a++)u[a]!==0&&(o[a*3]=l[a*3]/u[a],o[a*3+1]=l[a*3+1]/u[a],o[a*3+2]=l[a*3+2]/u[a]);if(!D)break}return{centroids:o,labels:B}}function W(e,r){const t=new Set;for(let i=0;i<r;i++)if(t.add(`${e[i*3]|0},${e[i*3+1]|0},${e[i*3+2]|0}`),t.size>64)return t.size;return t.size}function I(e,r,t,i){const n=new Float64Array(t*3),o=Math.floor(i()*r);n[0]=e[o*3],n[1]=e[o*3+1],n[2]=e[o*3+2];const B=new Float64Array(r).fill(1/0);for(let l=1;l<t;l++){let u=0;for(let a=0;a<r;a++){const C=e[a*3]-n[(l-1)*3],E=e[a*3+1]-n[(l-1)*3+1],g=e[a*3+2]-n[(l-1)*3+2],d=C*C+E*E+g*g;d<B[a]&&(B[a]=d),u+=B[a]}let A=i()*u,D=r-1;for(let a=0;a<r;a++)if(A-=B[a],A<=0){D=a;break}n[l*3]=e[D*3],n[l*3+1]=e[D*3+1],n[l*3+2]=e[D*3+2]}return n}const z=150;async function Y(e,r){const{width:t,height:i,data:n}=await N(e,r.stitchWidth),o=t*i,B=new Int16Array(o).fill(-1),l=new Int32Array(o);let u=0;for(let h=0;h<o;h++)n[h*4+3]>=z&&(l[u++]=h);if(u===0)return{width:t,height:i,cells:B,threads:[],counts:[]};const A=new Float64Array(u*3);for(let h=0;h<u;h++){const c=l[h]*4,y=k(n[c],n[c+1],n[c+2]);A[h*3]=y[0],A[h*3+1]=y[1],A[h*3+2]=y[2]}const{centroids:D,labels:a}=U(A,r.colorCount),C=D.length/3,E=Array.from({length:C},(h,c)=>[D[c*3],D[c*3+1],D[c*3+2]]),g=R(E,r.palette),d=new Array(C).fill(0);for(let h=0;h<u;h++)B[l[h]]=a[h],d[a[h]]++;return H({width:t,height:i,cells:B,threads:g,counts:d})}async function N(e,r){const t=await createImageBitmap(e),i=Math.max(1,Math.round(r)),n=Math.max(1,Math.round(i*t.height/t.width)),B=new OffscreenCanvas(i,n).getContext("2d",{willReadFrequently:!0});if(!B)throw t.close(),new Error("canvas 2d context unavailable");return B.imageSmoothingEnabled=!0,B.imageSmoothingQuality="high",B.drawImage(t,0,0,t.width,t.height,0,0,i,n),t.close(),{width:i,height:n,data:B.getImageData(0,0,i,n).data}}function x(e){return{width:e.width,height:e.height,cells:e.cells,threadNums:e.threads.map(r=>r.num),counts:e.counts}}function H(e){const r=e.threads.map((n,o)=>({i:o,key:K(n.rgb)})).sort((n,o)=>n.key[0]-o.key[0]||n.key[1]-o.key[1]||n.key[2]-o.key[2]).map(n=>n.i),t=new Int16Array(e.threads.length);r.forEach((n,o)=>{t[n]=o});const i=new Int16Array(e.cells.length);for(let n=0;n<e.cells.length;n++)i[n]=e.cells[n]<0?-1:t[e.cells[n]];return{...e,cells:i,threads:r.map(n=>e.threads[n]),counts:r.map(n=>e.counts[n])}}function K(e){const[r,t,i]=e.map(u=>u/255),n=Math.max(r,t,i),o=Math.min(r,t,i),B=n-o;let l=0;return B!==0&&(n===r?l=(t-i)/B%6:n===t?l=(i-r)/B+2:l=(r-t)/B+4,l/=6,l<0&&(l+=1)),[l,n===0?0:B/n,n]}self.onmessage=async e=>{const{id:r,photo:t,stitchWidth:i,colorCount:n,paletteNums:o}=e.data;try{const B=o?o.map(T).filter(A=>!!A):void 0,l=await Y(t,{stitchWidth:i,colorCount:n,palette:B}),u=x(l);self.postMessage({id:r,ok:!0,pattern:u},[u.cells.buffer])}catch(B){self.postMessage({id:r,ok:!1,error:B instanceof Error?B.message:String(B)})}}})();
