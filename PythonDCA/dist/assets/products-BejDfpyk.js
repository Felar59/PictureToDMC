...
}`;function w(t,e,a){const s=t.createShader(e);if(!s)throw new Error("could not create shader");if(t.shaderSource(s,a),t.compileShader(s),!t.getShaderParameter(s,t.COMPILE_STATUS)){const h=t.getShaderInfoLog(s);throw t.deleteShader(s),new Error(`shader failed to compile: ${h}`)}return s}function U(t){const e=t.getContext("webgl2",{antialias:!1,premultipliedAlpha:!1});if(!e)throw new Error("WebGL2 unavailable");const a=e.createProgram();if(!a)throw new Error("could not create program");const s=w(e,e.VERTEX_SHADER,C),h=w(e,e.FRAGMENT_SHADER,P);if(e.attachShader(a,s),e.attachShader(a,h),e.linkProgram(a),!e.getProgramParameter(a,e.LINK_STATUS))throw new Error(`program failed to link: ${e.getProgramInfoLog(a)}`);e.deleteShader(s),e.deleteShader(h);const f=e.createTexture();e.bindTexture(e.TEXTURE_2D,f),e.texParameteri(e.TEXTURE_2D,e.TEXTURE_MIN_FILTER,e.NEAREST),e.texParameteri(e.TEXTURE_2D,e.TEXTURE_MAG_FILTER,e.NEAREST),e.texParameteri(e.TEXTURE_2D,e.TEXTURE_WRAP_S,e.CLAMP_TO_EDGE),e.texParameteri(e.TEXTURE_2D,e.TEXTURE_WRAP_T,e.CLAMP_TO_EDGE);const i=o=>e.getUniformLocation(a,o),r={pattern:i("uPattern"),grid:i("uGrid"),resolution:i("uResolution"),cellPx:i("uCellPx"),origin:i("uOrigin"),clothColor:i("uClothColor"),groundAlpha:i("uGroundAlpha")},x=Object.keys(u).filter(o=>o!=="clothColor"),b=new Map(x.map(o=>[o,i(`u${o[0].toUpperCase()}${o.slice(1)}`)]));let m=null;return{render(o,g,y,S=!1,c){const v=Math.min(window.devicePixelRatio||1,2),l=Math.max(1,Math.round(c?c.width:t.clientWidth*v)),n=Math.max(1,Math.round(c?c.height:t.clientHeight*v));(t.width!==l||t.height!==n)&&(t.width=l,t.height=n),e.viewport(0,0,l,n),e.useProgram(a),e.enable(e.BLEND),e.blendFunc(e.SRC_ALPHA,e.ONE_MINUS_SRC_ALPHA),e.clearColor(0,0,0,0),e.clear(e.COLOR_BUFFER_BIT),e.activeTexture(e.TEXTURE0),e.bindTexture(e.TEXTURE_2D,f),o!==m&&(e.texImage2D(e.TEXTURE_2D,0,e.RGBA,o.width,o.height,0,e.RGBA,e.UNSIGNED_BYTE,o.data),m=o),e.uniform1i(r.pattern,0);const k=Math.min(l/o.width,n/o.height),d=Math.max(2,k*y);e.uniform2f(r.grid,o.width,o.height),e.uniform2f(r.resolution,l,n),e.uniform1f(r.cellPx,d),e.uniform2f(r.origin,(l-o.width*d)/2,(n-o.height*d)/2),e.uniform3fv(r.clothColor,g.clothColor),e.uniform1f(r.groundAlpha,S?0:1);for(const[E,T]of b)e.uniform1f(T,g[E]);e.drawArrays(e.TRIANGLES,0,3)},dispose(){e.deleteTexture(f),e.deleteProgram(a)}}}const A="/assets/cushion-mask-BC_L7swf.png",R="/assets/cushion-Shy2RVgn.avif",D="/assets/hoop-mask-B60I4cN8.png",_="/assets/hoop-DQZk-h_G.avif",L="/assets/shirt-mask-DhJb3ehr.png",G="/assets/shirt-DS87ZXGA.avif",I="/assets/tote-mask-B8IQIpus.png",F="/assets/tote-CYxjghuc.avif",O=.94,W=60,J={min:.28,max:2.6},X=[{key:"hoop",src:_,mask:D,aspect:1,spot:{x:.5,y:.5,w:.46}},{key:"tote",src:F,mask:I,aspect:900/704,spot:{x:.5,y:.54,w:.27,rot:-5}},{key:"shirt",src:G,mask:L,aspect:900/600,spot:{x:.47,y:.38,w:.135}},{key:"cushion",src:R,mask:A,aspect:1,spot:{x:.5,y:.5,w:.36}}];export{u as D,O as I,X as P,W as R,J as S,z as a,N as b,U as c,H as l,M as s};
