...
/**
 * The threads the marks are stitched in live in `./mark-palette`, which is
 * generated by scripts/export-dmc.py out of the same spreadsheet as the full
 * chart — the codes, and why those twelve, are in `MARK_CODES` there.
 *
 * They used to be resolved here, through `findThread`. That was correct and it
 * cost every visitor the whole 589-thread chart in the first bundle, because this
 * component is in the site header and can never be lazy — twenty kilobytes of
 * thread data to paint twelve colours in a 36px square. `mark-palette` is 1.2 kB
 * and says the same thing; scripts/check-mark-palette.mjs is what keeps it true,
 * and it matters more than it looks: the mark is picked by a modulo on the length
 * of this list, so one colour short would silently change every member's mark.
 */
